...
import Semantic.CCPrec.Syntax.CaptureSet
...
def Ty.rename_id {T : Ty sort s} : T.rename (Rename.id) = T := by
  ...
  case arrow ih1 ih2 =>
    ...
    exact congrArg (fun T => Ty.arrow T1 T) ih2
  case poly ih1 ih2 =>
    rename_i T1 T2
    simp only [Ty.rename, Rename.lift_id, ih1]
    exact congrArg (fun T => Ty.poly T1 T) ih2
  case cpoly cb ih =>
    rename_i cbound
    simp only [Ty.rename, Rename.lift_id, CaptureBound.rename_id]
    exact congrArg (fun T => Ty.cpoly cbound T) ih
  case unit => rfl
  case cap => rfl
  case bool => rfl
  case cell => rfl
