...
inductive Subtree.Intersect : Subtree -> Subtree -> CapKind -> Prop where
...
inductive CapKind.Intersect : CapKind -> CapKind -> CapKind -> Prop where
  | empty_l : Intersect [] K []
  | empty_r : Intersect K [] []
...
@[simp]
def Subtree.intersect (s : Subtree) (t : Subtree) : CapKind :=
...
theorem CapKind.intersect.cons_r :
    intersect [x] (y :: ys) = intersect [x] [y] ++ intersect [x] ys := by simp

theorem CapKind.intersect.append_r :
    intersect [x] (ys1 ++ ys2) = intersect [x] ys1 ++ intersect [x] ys2 := by simp

theorem CapKind.Intersect.lawful' : Intersect [x] L (.intersect [x] L) := by
  induction L
...
theorem CapKind.Intersect.lawful (K L : CapKind) : Intersect K L (K.intersect L) := by
...
theorem CapKind.intersect.top_r {K : CapKind} : K.intersect .top = K := by
  induction K
  case nil => simp
  case cons x xs ih =>
...
theorem CapKind.intersect.top_l {K : CapKind} : CapKind.top.intersect K = K := by
  induction K
  case nil => simp
  case cons y ys ih =>
    simp at *
...
theorem CapKind.intersect.assoc'' :
...
theorem CapKind.intersect.assoc' {B C : CapKind} :
...
theorem CapKind.intersect.assoc {A B C : CapKind} :
