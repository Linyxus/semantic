        ...
        · constructor
          ...
          · constructor
            · -- Prove expand_captures ... ⊆ Cf.denot env store
              ...
              simpa only [CaptureSet.denot, List.empty_eq] using
          ...
          · constructor
            ...
            · -- Show the polymorphic function property
              ...
              have hcap_rename :
                ...
                exact (rebind_captureset_denot (Rebind.tweaken (env:=env) (d:=denot)) Cf).symm
              -- Use monotonicity to relate capture set denotations at different memories
              have hCf_mono : Cf.denot env store = Cf.denot env m' := by
...
theorem sem_typ_pack
  ...
  intro env store hts
  simp only [Ty.exi_exp_denot, Ty.exi_val_denot, List.empty_eq]
  ...
  · simp only [Denot.as_mpost]
    ...
    · -- From ht, we have semantic typing for x at type T.subst (Subst.openCVar cs)
      ...
      simp only [Ty.exi_exp_denot, Ty.exi_val_denot, List.empty_eq] at hx
      ...
      case eval_var hQ =>
        ...
        simp only [Denot.as_mpost] at hQ
  ...
  | free fx =>
    ...
    have hheap :
      ...
      | some cell =>
        ...
        | val v =>
          change
            (match store.heap fx with
            | some (.val v') => some v'.unwrap
            | _ => none) = some (Exp.reader (.free y)) at hres
          rw [hmem] at hres
    ...
    case some cell =>
      ...
      | val hval =>
        ...
        | mk unwrap isVal reachability =>
          cases hv with
          | inl hl =>
            injection hl with hunwrap
            subst hunwrap
            use fx, true, isVal, reachability, rfl, hres
          | inr hr =>
            injection hr with hunwrap
            subst hunwrap
  ...
  case there b0 b hc_prev ih =>
    ...
    case var =>
      ...
      | .extend env' (.var x ps) =>
        ...
        obtain ⟨_, _, henv'⟩ := hts
        have hih := ih henv'
        rw [congrFun (rebind_capturebound_denot
            (Rebind.weaken (env := env') (x := x) (ps := ps)) cb') m] at hih
    ...
    case tvar =>
      ...
      | .extend env' (.tvar d) =>
        ...
        have hih := ih henv'
        rw [congrFun (rebind_capturebound_denot
            (Rebind.tweaken (env := env') (d := d)) cb') m] at hih
    ...
    case cvar =>
      ...
      | .extend env' (.cvar cs cap) =>
        ...
        have hih := ih henv'
        rw [congrFun (rebind_capturebound_denot
            (Rebind.cweaken (env := env') (cs := cs) (cap := cap)) cb') m] at hih
    ...
    case lock =>
      ...
      | .extend env' (.lock) =>
        ...
        obtain ⟨_, henv'⟩ := hts
        have hih := ih henv'
        rw [congrFun (rebind_capturebound_denot (Rebind.lweaken (env := env')) cb') m] at hih
...
theorem sem_typ_capp
  ...
  have hD'_wf : D'.WfInHeap store.heap := by
    exact CaptureSet.wf_subst (CaptureSet.wf_of_closed hD_closed) (from_TypeEnv_wf_in_heap hts)
...
theorem sem_typ_invoke
  ...
  simp only [Exp.subst, Subst.from_TypeEnv, Var.subst, Ty.exi_exp_denot, Ty.exi_val_denot,
    Ty.val_denot, CaptureSet.denot, List.empty_eq]
...
theorem sem_typ_unit :
  ...
  intro env store hts
  simpa only [Ty.exi_exp_denot, Exp.subst, List.empty_eq] using
    (Eval.eval_val (C := CaptureSet.denot env ∅ store) (m := store) (v := Exp.unit)
      Exp.IsVal.unit (by
...
theorem sem_typ_btrue :
  ...
  intro env store hts
  simpa only [Ty.exi_exp_denot, Exp.subst, List.empty_eq] using
    (Eval.eval_val (C := CaptureSet.denot env ∅ store) (m := store) (v := Exp.btrue)
      Exp.IsVal.btrue (by
        simp only [Denot.as_mpost, Ty.exi_val_denot, Ty.val_denot, resolve]
        left; trivial))
...
theorem sem_typ_bfalse :
  ...
  intro env store hts
  simpa only [Ty.exi_exp_denot, Exp.subst, List.empty_eq] using
    (Eval.eval_val (C := CaptureSet.denot env ∅ store) (m := store) (v := Exp.bfalse)
      Exp.IsVal.bfalse (by
        simp only [Denot.as_mpost, Ty.exi_val_denot, Ty.val_denot, resolve]
        right; trivial))
...
theorem sem_typ_cond
  ...
  have hsubC1 : CaptureSet.denot env C1 store ⊆ CaptureSet.denot env (C1 ∪ C2 ∪ C3) store := by
    ...
    exact CapabilitySet.Subset.trans
      CapabilitySet.Subset.union_right_left CapabilitySet.Subset.union_right_left
  have hguard := eval_capability_set_monotonic hguard_base hsubC1
  -- Assemble eval_cond
  suffices hcond :
      Eval (CaptureSet.denot env (C1 ∪ C2 ∪ C3) store) store
    ...
    simpa only [Ty.exi_exp_denot, Exp.subst, List.empty_eq] using hcond
  ...
  · -- true branch
    ...
    have hsubC2 : CaptureSet.denot env C2 m1 ⊆ CaptureSet.denot env (C1 ∪ C2 ∪ C3) m1 := by
      ...
      exact CapabilitySet.Subset.trans
        CapabilitySet.Subset.union_right_right CapabilitySet.Subset.union_right_left
  ...
  · -- false branch
    ...
    have hsubC3 : CaptureSet.denot env C3 m1 ⊆ CaptureSet.denot env (C1 ∪ C2 ∪ C3) m1 := by
      ...
      set B := CaptureSet.denot env C2 m1
      set C := CaptureSet.denot env C3 m1
      have hC : C ⊆ (A ∪ B) ∪ C := CapabilitySet.Subset.union_right_right
      simpa [A, B, C]
...
theorem sem_typ_reader
  ...
  simpa only [Ty.exi_exp_denot, Exp.subst, List.empty_eq] using
    (Eval.eval_val
      (C := CaptureSet.denot env ∅ store)
      (m := store)
      (v := Exp.reader ((Var.bound x).subst (Subst.from_TypeEnv env)))
      Exp.IsVal.reader
      (by
  ...
  suffices hread :
      Eval
        (((CaptureSet.var .epsilon (Var.bound x)).subst
          (Subst.from_TypeEnv env)).ground_denot store)
        store
        (Exp.read (.free (env.lookup_var x).1))
        (Ty.exi_val_denot env (.typ .bool)).as_mpost by
    simpa only [Ty.exi_exp_denot, Exp.subst, Subst.from_TypeEnv, Var.subst,
      CaptureSet.denot, List.empty_eq] using hread
  ...
  suffices hwrite :
      Eval
        (((CaptureSet.var .epsilon (Var.bound x)).subst
          (Subst.from_TypeEnv env)).ground_denot store)
        store
        (Exp.write (.free (env.lookup_var x).1) (.free (env.lookup_var y).1))
        (Ty.exi_val_denot env (.typ .unit)).as_mpost by
    simpa only [Ty.exi_exp_denot, Exp.subst, Subst.from_TypeEnv, Var.subst,
      CaptureSet.denot, List.empty_eq] using hwrite
...
theorem sem_typ_letin
  ...
  suffices hlet :
    ...
    simpa only [Ty.exi_exp_denot, Exp.subst, List.empty_eq] using hlet
  ...
  case h_val =>
    ...
    have hcap_rename :
      (C.rename Rename.succ).denot (env.extend_var l' ps)
      = C.denot env := by
      exact (rebind_captureset_denot (Rebind.weaken (env:=env) (x:=l') (ps:=ps)) C).symm
    ...
    · -- Show postcondition entailment
      exact Denot.imply_to_entails _ _
        (Denot.equiv_to_imply (weaken_exi_val_denot (env:=env) (x:=l') (ps:=ps) (T:=U))).2
    · -- Show: EnvTyping (Γ,x:T) (env.extend_var l')
      ...
      · -- Show: Ty.capt_val_denot env T
        ...
        have hQ1_lifted : Ty.val_denot env T
          ...
          val_denot_is_monotonic (typed_env_is_monotonic hts) T hext hQ1
        ...
        have hlookup : (m1.extend_val l' heapval hwf_v rfl hfresh).lookup l' =
          ...
          simpa [Memory.lookup, Memory.extend_val] using Heap.extend_lookup_eq m1.heap l' heapval
  ...
  case h_var =>
    ...
    case free fx =>
      ...
      have hcap_rename :
        ...
        = C.denot env := by
        exact (rebind_captureset_denot (Rebind.weaken (env:=env) (x:=fx) (ps:=ps)) C).symm
      ...
      · -- Show postcondition entailment
        exact Denot.imply_to_entails _ _
          (Denot.equiv_to_imply (weaken_exi_val_denot (env:=env) (x:=fx) (ps:=ps) (T:=U))).2
...
theorem sem_sc_elem {C1 C2 : CaptureSet s}
  ...
  case empty =>
    -- ∅.subst σ = ∅
    simp only [CaptureSet.subst, List.empty_eq]
    -- ∅.ground_denot m = ∅
    simp only [CaptureSet.ground_denot]
  ...
  case union_right_left ih =>
    simpa only [CaptureSet.subst, CaptureSet.ground_denot] using
...
theorem sem_sc_cvar {c : BVar s .cvar} {C : CaptureSet s}
  ...
  simp only [List.empty_eq]
  have hbound :
      ((env.lookup_cvar c).1.ground_denot m).BoundedBy (.set (C.denot env m)) := by
    simpa only [CaptureBound.denot] using typed_env_lookup_cvar_aux hts hlookup
  cases hbound with
  | set hsub =>
    exact hsub
...
theorem fundamental_subcapt
  ...
  SemSubcapt Γ C1 C2 := by
  induction hsub
  case sc_trans ih1 ih2 => exact sem_sc_trans ih1 ih2
      ...
      · constructor
        ...
        · constructor
          ...
          · -- Need to prove the body property with contravariant arg and covariant result
            ...
            have himply_entails :=
              Denot.imply_after_to_m_entails_after (hres (env.extend_var arg psT2) m'' htyping_ext)
        ...
        · constructor
          ...
          · -- Need to prove the body property with contravariant bound and covariant body
            ...
            have henv' : EnvTyping (Γ,C<:cb2)
                (env.extend_cvar CS (cap := CS.ground_denot m'')) m'' := by
              have henv'_base : EnvTyping (Γ,C<:cb2) (.extend env (.cvar CS (CS.ground_denot m'')))
                  m'' := by
                constructor
                · exact hCS_wf
                constructor
    ...
    | pack CS x =>
      ...
      · -- Construct EnvTyping for the extended context
        have henv' : EnvTyping (Γ,C<:.unbound)
            (env.extend_cvar CS (cap := CS.ground_denot m)) m := by
          have henv'_base :
              EnvTyping (Γ,C<:.unbound) (.extend env (.cvar CS (CS.ground_denot m))) m := by
            constructor
    ...
    · refine ⟨cs0, sepctx0, t0, hresolve, hcs0_wf, hsepctx0_wf, ?_, ?_, ?_⟩
      ...
      · intro m'' hsubm'' hkind hsep
        ...
        have htyping_lock : EnvTyping (Γ.push_lock Ψ) (env.extend_lock) m'' := by
          ...
          · exact htyping_m''
        have himply_entails :=
          Denot.imply_after_to_m_entails_after (hT (env.extend_lock) m'' htyping_lock)
        have heval1' :
            Eval (expand_captures m'.heap cs0) m'' t0
...
lemma resolve_pack_eq {e : Exp {}} {m : Memory} {CS : CaptureSet {}} {x : Var .var {}}
  ...
  rename_i cs y
  simp only [List.empty_eq] at hres
...
theorem resolve_is_pack {e : Exp {}} {m : Memory}
  ...
  -- Case analyze on e
  cases e <;> simp only [resolve, List.empty_eq] at hres
  case var y =>
    ...
    case free fy =>
      ...
      cases hval : m.heap fy <;> simp only [hval] at hres
      case none =>
        cases hres
      case some cell =>
        cases cell
        case val val =>
          ...
          injection hres with h
          subst h
          exact False.elim (simple_val_not_pack val.isVal hv)
        case capability cap =>
          simp only at hres
          cases hres
        case masked =>
  ...
  all_goals {
    -- After simp, hres identifies the resolved expression with v
    injection hres with h
    subst h
    exact hv
  }
...
theorem sem_typ_unpack
  ...
  case h_nonstuck =>
    ...
    · -- Prove v.WfInHeap m1.heap
      ...
      have heq : v = .pack CS x_pack :=
        resolve_pack_eq hres (resolve_is_pack hres Exp.IsPack.pack)
      ...
      · -- Prove x_pack.WfInHeap m1.heap
        ...
        have hwf_env : (env.extend_cvar CS (cap := CS.ground_denot m1)).is_implying_wf := by
          ...
          cases X with
          | there X' =>
            simpa only [TypeEnv.extend_cvar] using typed_env_is_implying_wf hts X'
  ...
  case h_val =>
    ...
    simp only [Denot.as_mpost] at hQ1
    ...
    case free fx =>
      ...
      simp only [List.empty_eq] at hQ1
      ...
      simp only [Ty.exi_exp_denot, List.empty_eq] at hu' ⊢
      ...
      have hexp_eq :
        ...
        rw [Exp.subst_comp]
        exact (congrArg (u.subst) (@Subst.from_TypeEnv_weaken_unpack s env cs fx ps)).trans
          (congrArg (u.subst) Subst.from_TypeEnv_extend_cvar_extend_var_cap_irrelevant)
      ...
      have heqv_composed : Ty.exi_val_denot env U ≈
        ...
        intro m e
        exact Iff.trans
          (rebind_exi_val_denot
            (Rebind.cweaken (env:=env) (cs:=cs) (cap:=cs.ground_denot m1)) U m e)
          (rebind_exi_val_denot
            (Rebind.weaken (env:=env') (x:=fx) (ps:=ps)) (U.rename Rename.succ) m e)
...
theorem fundamental
  ...
  case invoke =>
    ...
    | app hx_closed hy_closed =>
      ...
      exact sem_typ_invoke
        (ih_x (Exp.IsClosed.var Var.IsClosed.bound))
        (ih_y (Exp.IsClosed.var Var.IsClosed.bound))
  ...
  case cond ht1 ht2 ht3 ih1 ih2 ih3 =>
    ...
    | cond hclosed_guard hclosed_then hclosed_else =>
      exact sem_typ_cond
        (HasType.use_set_is_closed ht1) (HasType.use_set_is_closed ht2)
        (HasType.use_set_is_closed ht3) hclosed_guard hclosed_then hclosed_else
