...
theorem reduce_frame
  ...
  Reduce
    ...
    (base1 ++ inserted ++ (base2.rename_levels (frame_shift base1.len inserted.len) ++
      (extra.rename_levels (frame_shift base1.len inserted.len))))
    (e2.rename_levels (frame_shift base1.len inserted.len)) := by
  generalize heq1 : base1 ++ base2 = s_in at hr hwf_s hwf
  generalize heq2 : s_in ++ extra = s_out at hr
  induction hr generalizing base1 base2 inserted extra with
  | red_refl =>
    subst heq1
    have hnil : extra = Store.nil := Store.append_eq_self_iff_nil _ _ heq2
    subst hnil
    simp [Store.append_nil, Store.rename_levels]
    apply Reduce.red_refl
  | red_step hstep hrest ih =>
    have ⟨delta1, heq_mid⟩ := step_store_monotone hstep
    subst heq_mid
    have ⟨delta2, heq_out⟩ := reduce_store_monotone hrest
    subst heq_out
    subst heq1
    have hstep' := step_frame (inserted:=inserted) hwf_s hwf hstep
    apply Reduce.red_step hstep'
    have heq_ih : base1 ++ (base2 ++ delta1) = base1 ++ base2 ++ delta1 := by
      rw [Store.append_assoc]
    have hwf_s_ih := step_wf_store hwf_s hwf hstep
