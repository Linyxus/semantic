...
import Semantic.Capybara.Syntax.SepCtx
...
def Ty.rename_id {T : Ty sort s} : T.rename (Rename.id) = T := by
  ...
  | cap cs => simp only [Ty.rename, CaptureSet.rename_id]
...
theorem Ty.rename_comp {T : Ty sort s1} {f : Rename s1 s2} {g : Rename s2 s3} :
  ...
  | top => simp only [Ty.rename]
  | tvar x => simp only [Ty.rename, Rename.comp]
  | arrow T1 cs T2 ih1 ih2 =>
  ...
  | cap cs => simp only [Ty.rename, CaptureSet.rename_comp]
