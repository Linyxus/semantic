...
theorem exi_val_denot_typ
  (env : TypeEnv s) (T : Ty .capt s) :
    ⟦(Ty.typ T : Ty .exi s)⟧_[env] = ⟦T⟧_[env] := by
  change Ty.exi_val_denot env (Ty.typ T) = _
  exact Ty.exi_val_denot.eq_1 env T

/-- Unfolding lemma for existential-type val denotation (exi case). -/
theorem exi_val_denot_exi
  (env : TypeEnv s) (T : Ty .capt (s,C)) :
    ⟦(Ty.exi T : Ty .exi s)⟧_[env] = fun m e =>
    match resolve m.heap e with
    | some (.pack CS x) =>
      CS.WfInHeap m.heap ∧
      Ty.capt_val_denot (env.extend_cvar CS) T m (.var x)
    | _ => False := by
  change Ty.exi_val_denot env (Ty.exi T) = _
  exact Ty.exi_val_denot.eq_2 env T
  ...
  · -- cvar case
    ...
    case there c' =>
      ...
      case here =>
        ...
        rfl
      case there c0 =>
        -- Both sides reduce to ρ.lookup_cvar c0; traverse the ground capture set.
...
theorem exi_val_denot_is_transparent {env : TypeEnv s}
  ...
  | exi T =>
    ...
    have hlookup : m.heap x = some (Cell.val v) := by simp only [Memory.lookup] at hx; exact hx
          ...
          · intro m' denot msub hdenot_proper himply
            have henv' : (env.extend_tvar denot).IsMonotonic := by
              ...
              · intro X
                cases X with
                | here =>
                  simp only [TypeEnv.extend_tvar, TypeEnv.lookup_tvar]
                  intro C
        ...
        · -- Use monotonicity of capt_val_denot
          have henv' : (env.extend_cvar CS).IsMonotonic := by
            ...
            · intro X
              cases X with
              | there X' =>
                simp only [TypeEnv.extend_cvar, TypeEnv.lookup_tvar]
                exact henv.tvar X'
...
theorem shape_val_denot_is_reachability_safe {env : TypeEnv s}
  ...
  | cap =>
    ...
    have hcap' : m.heap label = some (Cell.capability .basic) := by
      simp only [Memory.lookup] at hcap
      exact hcap
...
theorem capt_denot_implyafter_lift
  ...
  simp only [Ty.capt_exp_denot] at heval ⊢
