...
lemma sem_subtyp_poly {S1 S2 : PureTy s} {cs1 cs2 : CaptureSet s} {T1 T2 : Ty .exi (s,X)}
  ...
  simp [Ty.val_denot] at h_poly_S1_cs1_T1 ⊢
        ...
        · constructor
          ...
          · -- Need to prove the body property with contravariant bound and covariant body
            ...
            have himply_S1 : denot.ImplyAfter m'' (Ty.val_denot env S1.core) := by
              ...
              simp [Denot.ImplyAfter, Denot.ImplyAt] at himply_S2
...
theorem sem_typ_unwrap
  ...
  (hx_syn : ∅ # Γ ⊢ Exp.var x : (Ty.modal cs Ψ E).typ)
  (hx : {} # Γ ⊨ Exp.var x : .typ (.modal cs Ψ E))
  ...
  have hx_closed := HasType.typed_var_closed hx_syn
  cases x with
  | free fx =>
    cases hx_closed
  | bound bx =>
...
theorem fundamental
  ...
  case unwrap =>
    ...
    exact sem_typ_unwrap hx (ih_x (Exp.IsClosed.var (HasType.typed_var_closed hx))) hkind hsep
  case invoke =>
    rename_i ih_x ih_y
