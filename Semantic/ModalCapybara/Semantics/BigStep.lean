    ...
    case capability info =>
      ...
      case mcell b' =>
        ...
        | false =>
          cases b' with
          | false =>
            simpa only [if_false] using hpred (by constructor) hsub hQ
          | true =>
            have hQ' : y .bfalse m2 := by
              simpa only [if_false] using hpred (by constructor) hsub hQ
            have hQ'' : y .btrue m2 := by
              rwa [hbool]
            simpa only [if_true] using hQ''
        | true =>
          cases b' with
          | false =>
            have hQ' : y .btrue m2 := by
              simpa only [if_true] using hpred (by constructor) hsub hQ
            have hQ'' : y .bfalse m2 := by
              rwa [← hbool]
            simpa only [if_false] using hQ''
          | true =>
            simpa only [if_true] using hpred (by constructor) hsub hQ
