...
theorem Cell.subsumes_trans {c1 c2 c3 : Cell}
  ...
  case capability.capability.capability info1 info2 info3 =>
    ...
    case basic.basic.mcell _ => exact h23
    case basic.mcell.mcell _ _ => cases h12
    case mcell.basic.basic _ => cases h12
    case mcell.mcell.basic _ _ => cases h23
  all_goals aesop
...
theorem resolve_reachability_monotonic
  ...
  | wf_pack _ _ => simp [resolve_reachability]
  | wf_app _ _ => simp [resolve_reachability]
  | wf_tapp _ _ => simp [resolve_reachability]
  | wf_capp _ _ => simp [resolve_reachability]
  | wf_letin _ _ => simp [resolve_reachability]
  | wf_unpack _ _ => simp [resolve_reachability]
  | wf_unit => simp [resolve_reachability]
  | wf_btrue => simp [resolve_reachability]
  | wf_bfalse => simp [resolve_reachability]
  | wf_read _ => simp [resolve_reachability]
  | wf_write _ _ => simp [resolve_reachability]
  | wf_cond _ _ _ => simp [resolve_reachability]
...
theorem compute_reachability_monotonic
  ...
  | reader =>
    rename_i x
    cases x with
    | free loc => simp [compute_reachability]
    | bound bx => cases bx
  | unit =>
    -- Case: v = .unit
    ...
    | free loc =>
      simpa only [expand_captures] using
        congrArg (CapabilitySet.applyMut m)
          (reachability_of_loc_update_mcell h l hexists b loc)
  | union cs1 cs2 ih1 ih2 =>
...
theorem compute_reachability_update_mcell (h : Heap) (l : Nat)
  ...
  cases hv with
  | abs =>
    simpa only [compute_reachability] using expand_captures_update_mcell h l hexists b _
  | tabs =>
    simpa only [compute_reachability] using expand_captures_update_mcell h l hexists b _
  | cabs =>
    simpa only [compute_reachability] using expand_captures_update_mcell h l hexists b _
...
theorem update_mcell_subsumes_compat {m1 m2 : Memory} (l : Nat) (b : Bool)
  ...
  change (m2.heap.update_cell l (.capability (.mcell b))).subsumes
    (m1.heap.update_cell l (.capability (.mcell b)))
  ...
  · rw [if_neg hneq] at hlookup
    obtain ⟨v', hv', hsub_v⟩ := hsub l' v hlookup
    refine ⟨v', ?_, hsub_v⟩
    rw [if_neg hneq]
    exact hv'
